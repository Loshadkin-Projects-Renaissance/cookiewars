...
# TODO: /silent off/on
@bot.message_handler(commands=['silenton'], func=creator_command)
def silent_on_handler(m):
    global hidetext
    hidetext = 1
    bot.send_message(m.chat.id, 'Silent mode is ON.')


@bot.message_handler(commands=['give'], func=lambda m: creator_command(m) and reply_command(m) and argument_command(m))
def give_handler(m):
    user = users.find_one({'id': m.reply_to_message.from_user.id})
    if not user:
        return
    users.update_one({'id': user['id']}, {'$push': {'bot.bought': m.text.split(' ')[1]}})
    bot.send_message(m.chat.id, 'Теперь у ' + user['name'] + ' есть ' + m.text.split(' ')[1] + '!')


@bot.message_handler(commands=['silentoff'], func=creator_command)
def silent_off_handler(m):
    global hidetext
    hidetext = 0
...
def referal_handler(m):
    bot.send_message(m.chat.id,
                     'Присоединяйся к игре CookieWars! Прокачай своего бойца, отправь в бой и наслаждайся тем, как он сам уничтожает соперника!\n' +
                     'https://telegram.me/cookiewarsbot?start=' + str(m.from_user.id))


@bot.message_handler(commands=['nextgame'])
def nextgame_handler(m):
...
def top_handler(m):
    ...
    for i in range(10):
        ...
        for ids in lst:
            myexp = 0
            for g in ids['botslots']:
                if ids['botslots'][g] != {}:
                    myexp += ids['botslots'][g]['exp']
            myexp += ids['bot']['exp']
            if myexp > dieturn:
...
@bot.message_handler(commands=['giftadmin'], func=lambda m: creator_command(m) and reply_command(m))
def giftadmin_handler(m):
    user = users.find_one({'id': m.reply_to_message.from_user.id})
    if not user:
        return
    users.update_one({'id': user['id']}, {'$push': {'bot.bought': 'gift'}})
    bot.send_message(m.chat.id, 'Теперь ' + user['name'] + ' гифт-админ!')


@bot.message_handler(commands=['offgames'], func=creator_command)
def offgames_handler(m):
    variables.update_one({'vars': 'main'}, {'$set': {'enablegames': 0}})
    bot.send_message(m.chat.id, 'Режим технических работ включён!')


@bot.message_handler(commands=['ongames'], func=creator_command)
def ongames_handler(m):
    variables.update_one({'vars': 'main'}, {'$set': {'enablegames': 1}})
    bot.send_message(m.chat.id, 'Режим технических работ выключен!')


@bot.message_handler(commands=['dropname'], func=lambda m: creator_command(m) and reply_command(m))
def dropname_handler(m): 
...
def autojoin_handler(m):
    if m.from_user.id == m.chat.id:
        enable = '☑️'
        enablen = '☑️'
        user = users.find_one({'id': m.from_user.id})
        if user['enablejoin'] == 1:
            enable = '✅'
        if user['nomutantjoin'] == 1:
            enablen = '✅'
...
def invent_handler(m):
    if m.from_user.id == m.chat.id:
        ...
        for item in x['bot']['bought']:
            ...
            if item == 'magictitan':
                kb.add(
                    types.InlineKeyboardButton(text=magictitan + '🔵Магический титан', callback_data='equipmagictitan'))
            if item == 'turret':
            ...
            if item == 'metalarmor':
                kb.add(types.InlineKeyboardButton(text=metalarmor + '🔲Металлическая броня',
                                                  callback_data='equipmetalarmor'))
            if item == 'electrocharge':
                kb.add(types.InlineKeyboardButton(text=electrocharge + '🔋Электрический снаряд',
                                                  callback_data='equipelectrocharge'))
            if item == 'secrettech':
                kb.add(types.InlineKeyboardButton(text=secrettech + '⁉Секретные технологии',
                                                  callback_data='equipsecrettech'))
...
def upgrade_handler(m):
    if m.chat.id == m.from_user.id:
        kb = types.InlineKeyboardMarkup()
        kb.add(types.InlineKeyboardButton(text='ХП', callback_data='hp'),
               types.InlineKeyboardButton(text='Урон', callback_data='dmg'),
               types.InlineKeyboardButton(text='Прочее', callback_data='different'))
        kb.add(types.InlineKeyboardButton(text='Вампиризм', callback_data='vampirizm'),
...
def name_handler(m):
    arguments = m.text.split(' ')
    if not len(arguments):
        bot.send_message(m.chat.id, 'Вы не ввели имя.')
        return
    _, name = arguments
    # TODO: Я не закінчив рефакторити цю жесть, не чипати
    if not validate_name(name):
        bot.send_message(m.chat.id,
                        'Имя должно быть от 2 до 18 символов, состоять из циферок или буковок.')
        if '@' not in arguments[1]:
            no = 0
            for ids in arguments[1]:
                ...
                for ids in y:
                    ...
                    while i <= 3:
                        try:
                            ...
                            pass
                        i += 1
                if arguments[1] not in allnames:
                    x = users.find_one({'id': m.from_user.id})
                    users.update_one({'id': m.from_user.id}, {'$set': {'bot.name': arguments[1]}})
                    bot.send_message(m.chat.id, 'Вы успешно изменили имя бойца на ' + arguments[1] + '!')
                else:
...
def buyslot_handler(m):
    ...
    if no == 0:
        ...
        bot.send_message(m.chat.id,
                         'Изначально у вас есть 1 свободный слот для бойца помимо базового. Есть 2 способа покупки новых слотов:\n' +
                         ...
                         '*ВНИМАНИЕ!!!* За одну покупку нельзя получить сразу 2 слота, это должны быть 2 разны платежа!',
                         reply_markup=kb, parse_mode='markdown')


@bot.message_handler(commands=['dnashop'])
def dnashop_handler(m):
    x = users.find_one({'id': m.from_user.id})
    if m.from_user.id == m.chat.id:
        dnamenu(x)
    else:
        ...
        try:
            ...
            if n > 0:
                ...
                if x['cookie'] >= cost:
                    ...
                    bot.send_message(m.chat.id,
                                     str(n) + ' ДНК успешно добавлены в очередь на производство! Я сообщу вам, когда всё будет готово.')
        except:
...
def inline(call):
    try:
        ...
        if 'dna' in call.data:
            ...
            if call.data == 'dna buy':
                if 'dnagenerator' in x['buildings']:
                    medit('Выберите количество ДНК, которое хотите произвести. На производство одной единицы ' +
                          ...
                          'Для этого напишите следующую команду:\n/createdna *количество*', call.message.chat.id,
                          call.message.message_id, parse_mode='markdown')
                else:
                    medit('Чтобы производить ДНК, вам нужно купить строение - "ДНК-генератор".', call.message.chat.id,
                ...
                if x['cookie'] >= cost:
                    ...
                    medit('Вы успешно купили слот для ' + build + 'го бойца!', call.message.chat.id,
                          call.message.message_id)
                else:
                ...
                if 'cloner' in x['buildings']:
                    ...
                    while i <= 3:
                        ...
                        i += 1
                    kb = types.InlineKeyboardMarkup()
                    kb.add(types.InlineKeyboardButton('Клонировать (свободных слотов осталось: ' + str(slots) + ')',
                                                      callback_data='dna clonebot'))
                    medit(
                        'Чтобы клонировать своего бойца, нажмите на кнопку ниже. Стоимость: 1🧬. По завершению клонирования ' +
                        ...
                        'переключаться между бойцами.\nДля покупки новых слотов введите /buyslot.',
                ...
                if x['dna'] >= 1:
                    ...
                    if slots > 0:
                        ...
                        users.update_one({'id': x['id']},
                                         {'$set': {'botslots.' + cbot + '.bought': x['bot']['bought']}})
                        ...
                        medit('Запускаю клонирователь...\n' +
                              ...
                              'console: progress: 1%_', call.message.chat.id, call.message.message_id,
                              parse_mode='markdown')
                        bot.send_message(x['id'],
                                         '_console: progress: 100%. Copy of your fighter is ready! Thank you for using "PenisDetrov" ' +
                                         'technology!_\n\nЧтобы поменять текущего бойца на другого, нажмите /selectbot.',
                                         parse_mode='markdown')
                    else:
                        medit('У вас нет доступных слотов! Для покупки введите /buyslot.', call.message.chat.id,
                              call.message.message_id)
                else:
                ...
                medit(
                    'ДНК-генератор - самое важное строение на пути к усовершенствованию генокода вашего бойца. Оно позволит вам производить ДНК-очки, ' +
                    'которые понадобятся для разработки способностей нового поколения.', call.message.chat.id,
                ...
                medit(
                    'Клонирователь - устройсво, которое позволит вам сделать усовершенствованную копию своего бойца, тело которого сможет принять ' +
                    'те мутации, которые вы разработаете! Некоторые из них необратимы, поэтому в будущем вам понадобится ' +
                    'купить дополнительные слоты для хранения копий бойца.', call.message.chat.id,
                    call.message.message_id, reply_markup=kb)
                ...
                medit(
                    'Выберите мутацию, которую хотите изучить. Изучив мутацию 1 раз, вы сможете применять её к любому количеству бойцов.',
                    call.message.chat.id, call.message.message_id, reply_markup=kb)
                ...
                medit(
                    'Оборотень - это человек-волк, который имеет все преимущества обеих личностей. Каждый чётный ход вы будете превращаться в ' +
                    'волка, который имеет 30% уворота и вампиризм (при успешной атаке восстанавливает себе хп). ' +
                    'В дальнейшем вы сможете улучшить это ДНК, добавляя новые способности и усиляя предыдущие.',
                    call.message.chat.id, call.message.message_id, reply_markup=kb)
                ...
                kb.add(types.InlineKeyboardButton(text=tx + ' (' + str(cost) + '🧬)', callback_data=data))
                medit(
                    '(пока что не дает баффов, разрабатывается...) Электродемон - нечисть, питающаяся электричеством. После получения его ДНК, боец обретёт силу, которая и не снилась ' +
                ...
                if x['dna'] >= cost:
                    ...
                    medit('_console: Недостаточно 🧬ДНК!_', call.message.chat.id, call.message.message_id,
                          parse_mode='markdown')
                ...
                if no == 0:
                    if 'mutant' in x['bot']['mutations']:
                        ...
                        medit('Даём бойцу инъекцию с ДНК ' + text + '! Отойдём подальше, мало ли что...\n...\n' +
                              'Готово! Чтобы увидеть мутацию в действии, сыграйте матч и посмотрите за результатом.',
                              call.message.chat.id, call.message.message_id, parse_mode='markdown')
                    else:
                        medit(
                            'Нельзя мутировать начального бойца! Создайте его улучшенную копию с помощью клонирователя!',
                            call.message.chat.id, call.message.message_id, parse_mode='markdown')
                else:
                    medit('Этот боец уже имеет мутацию!', call.message.chat.id, call.message.message_id,
                          parse_mode='markdown')
            ...
            kb.add(
                types.InlineKeyboardButton(text=electrocharge + '🔋Электрический заряд', callback_data='electrocharge'))
            ...
            medit(
                'Каждый ход у вас есть 25% шанс прибавить полученный вами в этом раунде урон к силе атаки. Хотите приобрести?',
                call.message.chat.id, call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit(
                'Каждый раз, как вы атакуете соперника, у вас есть 20% шанс нанести критический урон, зависящий от вашей энергии перед выстрелом. Хотите приобрести?',
                call.message.chat.id, call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit(
                'В конце хода вы блокируете одну единицу урона со 100% шансом, но шанс попасть по вам увеличивается на 8%. Хотите приобрести?',
                call.message.chat.id, call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit('Вы начинаете матч с одним из трёх техно-оружий. Хотите приобрести?', call.message.chat.id,
                  call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit(
                'В начале матча вы ставите турель. В конце каждого хода она имеет 40% шанс выстрелить по случайному сопернику нанеся 1 урона, и 25% шанс поджечь его на 2 хода. Хотите приобрести?',
                call.message.chat.id, call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit('Генератор щитов каждые 6 хода даёт боту щит. Хотите приобрести?', call.message.chat.id,
                  call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit(
                'Ваш боец теряет половину хп, и создаёт копию себя с отнятыми жизнями и со всеми вашими скиллами (кроме двойника и инженера). Хотите приобрести?',
                call.message.chat.id, call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit('Оружие вашего бойца меняется на волшебную палочку в начале боя. Хотите приобрести?',
                  call.message.chat.id, call.message.message_id, reply_markup=kb)
            ...
            medit('Имеет 18% шанс активироваться. Весь полученный на этом ходу урон уменьшается в 2 раза, а ' + \
                  'атаковавшие вас соперники загораются на 3 хода, включая текущий. Хотите приобрести?',
                  call.message.chat.id, call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit(
                'Когда цель, которую вы атакуете, теряет хп, вы имеете 65% шанс прибавить это хп к монстру, которого призовёте после смерти. Ваши хп в начале матча уменьшены на 1. Хотите приобрести?',
                call.message.chat.id, call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit(
                'Теперь вы - магический титан! Имеете 6 маны. Пока у вас есть мана, вы неуязвимы. Имеете 50% шанс заблокировать входящий урон. 1 мана тратится на блокировку 1 урона. ' + \
                'Когда мана заканчивается, вы теряете 1 хп и восстанавливаете ману. Хотите приобрести?',
                call.message.chat.id, call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit(
                'Этот скилл даёт боту возможность восстанавливать себе 1 хп каждые 9 ходов с шансом 75%, но имеет 25% шанс потерять хп вместо восстановления. Хотите приобрести?',
                call.message.chat.id, call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit(
                'Этот скилл даёт боту 2 доп. хп в начале матча, но уменьшает шанс попасть из любого оружия на 20%. Хотите приобрести?',
                call.message.chat.id, call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit('Этот скилл увеличивает порог урона на 3. Хотите приобрести?', call.message.chat.id,
                  call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit('Шанс попасть по бойцу сокращается на 20%. Хотите приобрести?', call.message.chat.id,
                  call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit('Этот скилл увеличивает шанс попадания из любого оружия на 30%. Хотите приобрести?',
                  call.message.chat.id, call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit('Этот скилл позволяет убить врага, у которого остался 1 хп, не смотря ни на что. Хотите приобрести?',
                  call.message.chat.id, call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit('Позволяет с шансом 3% в конце каждого хода отнять всем соперникам 1 хп. Хотите приобрести?',
                  call.message.chat.id, call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit(
                'После своей смерти воин живёт еще 2 хода, получая +3 урона к атакам, а затем умирает. Хотите приобрести?',
                call.message.chat.id, call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit('Если применить на атакующего врага, он атакует сам себя. Хотите приобрести?', call.message.chat.id,
                  call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit(
                'Хп бойца снижено на 2. После смерти боец призывает разьяренного паука, у которого 3 хп. Хотите приобрести?',
                call.message.chat.id, call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit('Если хп опускается ниже 3х, ваш урон повышается на 2. Хотите приобрести?', call.message.chat.id,
                  call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit(
                'В начале матча этот куб превращается в случайный скилл. Можно купить, не покупая предыдущие улучшения. Хотите приобрести?',
                call.message.chat.id, call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit('Если боец атаковал и отнял хп у врага, с шансом 9% он восстановит себе 1 хп. Хотите приобрести?',
                  call.message.chat.id, call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit(
                'Когда боец умирает, он имеет 60% шанс отнять 1хп случайному врагу. Если при этом враг умрет, маг воскреснет с 2хп, а убитый станет зомби. За бой может быть использовано многократно. Хотите приобрести?',
                call.message.chat.id, call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit('Скин позволяет воину с 30% шансом избежать фатального урона один раз за игру. Хотите приобрести?',
                  call.message.chat.id, call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit(
                'Увеличивает шансы применения всех пассивных скиллов на 20% (для примера: шанс применить титана был 50%, а станет 60%). Хотите приобрести?',
                call.message.chat.id, call.message.message_id, reply_markup=kb)
            ...
            kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
            medit('Скин увеличивает максимальный уровень энергии бойца на 2. Хотите приобрести?', call.message.chat.id,
                  call.message.message_id, reply_markup=kb)
            ...
            if 'electrocharge' not in x['bot']['bought']:
                if x['cookie'] >= 4700:
                    if 'suit' in x['bot']['bought']:
                        ...
                        medit('Вы успешно приобрели скилл "Электрический снаряд"!', call.message.chat.id,
                              call.message.message_id)
                    else:
                        bot.answer_callback_query(call.id, 'Сначала приобретите предыдущее улучшение!')
                else:
            ...
            if 'metalarmor' not in x['bot']['bought']:
                if x['cookie'] >= 5300:
                    if 'electrocharge' in x['bot']['bought']:
                        ...
                        medit('Вы успешно приобрели скилл "Металлическая броня"!', call.message.chat.id,
                              call.message.message_id)
                    else:
                        bot.answer_callback_query(call.id, 'Сначала приобретите предыдущее улучшение!')
                else:
            ...
            if 'secrettech' not in x['bot']['bought']:
                if x['cookie'] >= 10000:
                    if 'turret' in x['bot']['bought']:
                        ...
                        users.update_one({'id': call.from_user.id}, {'$inc': {'cookie': -10000}})
                        medit('Вы успешно приобрели скилл "Секретные технологии"!', call.message.chat.id,
                              call.message.message_id)
                    else:
                        bot.answer_callback_query(call.id, 'Сначала приобретите предыдущее улучшение!')
                else:
            ...
            if 'suit' not in x['bot']['bought']:
                if x['cookie'] >= 4200:
                    ...
                    medit('Вы успешно приобрели скилл "Отражающий костюм"!', call.message.chat.id,
                          call.message.message_id)
                else:
                    bot.answer_callback_query(call.id, 'Недостаточно поинтов!')
            else:
            ...
            if 'firemage' not in x['bot']['bought']:
                if x['cookie'] >= 5500:
                    if 'mage' in x['bot']['bought']:
                        ...
                        medit('Вы успешно приобрели скилл "Повелитель огня"!', call.message.chat.id,
                              call.message.message_id)
                    else:
                        bot.answer_callback_query(call.id, 'Сначала приобретите предыдущее улучшение!')
                else:
            ...
            if 'magictitan' not in x['bot']['bought']:
                if x['cookie'] >= 7000:
                    if 'necromant' in x['bot']['bought']:
                        ...
                        medit('Вы успешно приобрели скилл "Магический титан"!', call.message.chat.id,
                              call.message.message_id)
                    else:
                        bot.answer_callback_query(call.id, 'Сначала приобретите предыдущее улучшение!')
                else:
                    ...
                    if len(x['bot']['skills']) <= 1:
                        ...
                        try:
                            bot.answer_callback_query(call.id,
                                                      'Вы успешно экипировали скилл "' + skilltoname(txt[1]) + '"!')
                        except:
                            bot.answer_callback_query(call.id, 'Вы успешно экипировали скилл "' + 'Неизвестно' + '"!')
                    else:
                        bot.answer_callback_query(call.id,
                                                  'У вас уже экипировано максимум скиллов(2). Чтобы снять скилл, нажмите на его название.')
                else:
            ...
            y = users.find_one({'id': call.from_user.id})
            kb = types.InlineKeyboardMarkup()
            kb.add(types.InlineKeyboardButton(text='+1🤖', callback_data='+1'),
                   types.InlineKeyboardButton(text='+2🤖', callback_data='+2'),
                   types.InlineKeyboardButton(text='+5🤖', callback_data='+5'))
            kb.add(types.InlineKeyboardButton(text='+10🤖', callback_data='+10'),
            ...
            if y['currentjoinbots'] * 20 <= y['cookie']:
                ...
                medit('Вы успешно приобрели ' + str(x) + '🤖 джойн-ботов!', call.message.chat.id,
                      call.message.message_id)
            else:
                medit('Недостаточно поинтов!', call.message.chat.id, call.message.message_id)
            ...
            if x['nomutantjoin'] == 0:
                ...
                medit('🚫Автоджоин к играм без мутантов успешно выключен!', call.message.chat.id,
                      call.message.message_id)
                    ...
                    for ids in enemy:
                        if ids['die'] != 1 and ids['zombie'] <= 0:
                            if ids['identeficator'] != None:
                                ...
                                x = ids['realid']
                            kb.add(types.InlineKeyboardButton(text=ids['name'],
                                                              callback_data='fight selecttarget ' + str(
                                                                  chat) + ' ' + str(x)))
                    ...
                    for ids in me['skills']:
                        if ids in usable:
                            if ids == 'gipnoz' and me['gipnoz'] <= 0:
                                kb.add(types.InlineKeyboardButton(text=skilltoname(ids),
                                                                  callback_data='fight use ' + str(chat) + ' ' + ids))
                    ...
                    for ids in me['mutations']:
                        if ids in usable:
                            if ids == 'electro' and me['shockcd'] <= 0:
                                kb.add(types.InlineKeyboardButton(text='🔌Разряд!', callback_data='fight use ' + str(
                                    chat) + ' ' + 'electro'))
                    kb.add(types.InlineKeyboardButton(text='Назад', callback_data='fight back ' + str(chat)))
                    ...
                    if skill != 'medic':
                        ...
                        for ids in enemy:
                            if ids['identeficator'] != None:
                                ...
                                x = ids['realid']
                            kb.add(types.InlineKeyboardButton(text=ids['name'],
                                                              callback_data='fight skilltarget ' + str(
                                                                  chat) + ' ' + str(x) + ' ' + skill))
...
def givekeyboard(id, user):
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton(text='⚔️Атака', callback_data='fight attackchoice ' + str(id)),
           types.InlineKeyboardButton(text='🕑Перезарядка', callback_data='fight reload ' + str(id)))
    kb.add(types.InlineKeyboardButton(text='💨Уворот', callback_data='fight yvorot ' + str(id)),
           types.InlineKeyboardButton(text='⭐️Скиллы', callback_data='fight skills ' + str(id)))
    ...
    for ids in games[id]['bots']:
        ...
        if i > 100 and user['prize1'] == 0:
            if user['inviter'] != None:
                ...
                try:
                    bot.send_message(user['inviter'], 'Ваш приглашённый игрок ' + user[
                        'name'] + ' получил ранг "Эсквайр"! Вы получаете ' + str(int(prize1 / 2)) + '⚛️.')
                except:
                    pass
            try:
        ...
        if i > 500 and user['prize2'] == 0:
            if user['inviter'] != None:
                ...
                try:
                    bot.send_message(user['inviter'], 'Ваш приглашённый игрок ' + user[
                        'name'] + ' получил ранг "Солдат"! Вы получаете ' + str(int(prize2 / 2)) + '⚛️.')
                except:
                    pass
            try:
        ...
        if i > 800 and user['prize3'] == 0:
            if user['inviter'] != None:
                ...
                try:
                    bot.send_message(user['inviter'], 'Ваш приглашённый игрок ' + user[
                        'name'] + ' получил ранг "Опытный боец"! Вы получаете ' + str(int(prize3 / 2)) + '⚛️.')
                except:
                    pass
            try:
        ...
        if i > 2000 and user['prize4'] == 0:
            if user['inviter'] != None:
                ...
                try:
                    bot.send_message(user['inviter'], 'Ваш приглашённый игрок ' + user[
                        'name'] + ' получил ранг "Подполковник"! Вы получаете ' + str(int(prize4 / 2)) + '⚛️.')
                except:
                    pass
            try:
        ...
        if i > 3500 and user['prize5'] == 0:
            if user['inviter'] != None:
                ...
                try:
                    bot.send_message(user['inviter'], 'Ваш приглашённый игрок ' + user[
                        'name'] + ' получил ранг "Генерал"! Вы получаете ' + str(int(prize5 / 2)) + '⚛️.')
                except:
                    pass
            try:
        ...
        if i > 7000 and user['prize6'] == 0:
            if user['inviter'] != None:
                ...
                try:
                    bot.send_message(user['inviter'], 'Ваш приглашённый игрок ' + user[
                        'name'] + ' получил ранг "Повелитель"! Вы получаете ' + str(int(prize6 / 2)) + '⚛️.')
                except:
                    pass
            try:
        ...
        if i > 50000 and user['prize7'] == 0:
            if user['inviter'] != None:
                ...
                try:
                    bot.send_message(user['inviter'], 'Ваш приглашённый игрок ' + user[
                        'name'] + ' получил ранг "Бог"! Вы получаете ' + str(int(prize7 / 2)) + '⚛️.')
                except:
                    pass
            try:
        ...
        if i > 100000 and user['prize8'] == 0:
            if user['inviter'] != None:
                ...
                try:
                    bot.send_message(user['inviter'], 'Ваш приглашённый игрок ' + user[
                        'name'] + ' получил ранг "Пасюк"! Вы получаете ' + str(int(prize8 / 2)) + '⚛️.')
                except:
                    pass
            try:
        ...
        if i > 250000 and user['prize9'] == 0:
            if user['inviter'] != None:
                ...
                try:
                    bot.send_message(user['inviter'], 'Ваш приглашённый игрок ' + user[
                        'name'] + ' получил ранг "Сверхразум"! Вы получаете ' + str(int(prize9 / 2)) + '⚛️.')
                except:
                    pass
            try:
        ...
        if i > 666666 and user['prize10'] == 0:
            if user['inviter'] != None:
                ...
                try:
                    bot.send_message(user['inviter'], 'Ваш приглашённый игрок ' + user[
                        'name'] + ' получил ранг "Дьявол"! Вы получаете ' + str(int(prize10 / 2)) + '⚛️.')
                except:
                    pass
            try:
        ...
        if i > 1000000 and user['prize11'] == 0:
            if user['inviter'] != None:
                ...
                try:
                    bot.send_message(user['inviter'], 'Ваш приглашённый игрок ' + user[
                        'name'] + ' получил ранг "Высшее существо"! Вы получаете ' + str(int(prize11 / 2)) + '⚛️.')
                except:
                    pass
            try:
        ...
        if 'electrocharge' in bots['skills'] and bots['attack'] == 1:
            ...
            if x == 1:
                ...
                if random.randint(1, 100) <= 20 * (bots['chance'] + 1):
                    ...
                    if dmg < 0:
                        dmg = 0
                    bots['doptext'] += '🔋' + bots['name'] + ' заряжает свою атаку! Соперник получает ' + str(
                        dmg) + ' дополнительного урона!\n'
    ...
    if 0 not in games[id]['bots']:
        ...
        for ids in games[id]['bots']:
            if games[id]['bots'][ids]['die'] != 1 and games[id]['bots'][ids]['id'] not in dead:
                alive += 1
        if ((die + 1 >= len(games[id]['bots']) or len(allid) <= 1) and games[id]['mode'] != 'farm') or (
                (games[id]['mode'] == 'farm' and games[id]['xod'] >= endxoda) or alive == 0):
            z = 1
            if games[id]['mode'] == 'farm':
                ...
                winid = []
                for ids in games[id]['bots']:
                    if games[id]['bots'][ids]['die'] != 1 and games[id]['bots'][ids]['id'] not in winid and \
                            games[id]['bots'][ids]['id'] not in dead:
                ...
                ftext = 'Режим "Пекло":\nВсе выжившие получают награду в размере: ' + str(
                    points) + '⚛️!\nСписок выживших:\n' + slist
                z = 1
                ...
                if name != None:
                    ...
                    for ids in games[id]['bots']:
                        try:
                            ...
                            points += 4
                    for ids in games[id]['bots']:
                        if 'werewolf' in games[id]['bots'][ids]['mutations'] or 'electro' in games[id]['bots'][ids][
                            'mutations']:
                            points += 18
                    for ids in games[id]['bots']:
                    ...
                    while i < 3:
                        ...
                        for ids in games[id]['bots']:
                            if winner != None:
                                if games[id]['bots'][ids]['dieturn'] > dieturn and games[id]['bots'][
                                    ids] not in place and games[id]['bots'][ids]['id'] != winner['id'] and \
                                        games[id]['bots'][ids]['id'] not in idlist and games[id]['bots'][ids][
                                    'name'] != 'Редкий слизнюк':
                                    a = games[id]['bots'][ids]
                    ...
                    if winner['id'] != 0:
                        ...
                        if True:
                            if games[id]['mode'] == None:
                                ...
                                try:
                                    ...
                                    if name != 'Редкий слизнюк':
                                        bot.send_message(id,
                                                         '🏆' + yy + name + ' победил' + zz + '! Он получает ' + str(
                                                             points) + '❇️ опыта, а ' + winner2['name'] + ' - ' + str(
                                                             points) + '⚛️ поинтов и ' + str(
                                                             cookie) + '🍪 куки;\n' + txt + 'Все участники игры получают 2⚛️ поинта и 2❇️ опыта!')
                                        try:
                                            bot.send_message(winner2['id'],
                                                             '🏆' + yy + name + ' победил' + zz + '! Он получает ' + str(
                                                                 points) + '❇️ опыта, а ' + winner2[
                                                                 'name'] + ' - ' + str(points) + '⚛️ поинтов и ' + str(
                                                                 cookie) + '🍪 куки;\nВсе участники игры получают 2⚛️ поинта и 2❇️ опыта!')
                                        except:
                                            pass
                                        userstrug.update_one({'id': winner['id']}, {
                                            '$inc': {'cookies': cookie, 'totalcookies.cwcookies': cookie}})
                                        users.update_one({'id': winner['id']}, {'$inc': {'dailycookie': cookie}})
                                    else:
                                        bot.send_message(id, 'Редкий слизнюк сбежал!')
                                except:

                                    bot.send_message(id, '🏆' + name + ' победил! Он получает ' + str(
                                        points) + '❇️ опыта, а ' + winner2['name'] + ' - ' + str(
                                        points) + '⚛️ поинтов! Куки получить не удалось - для этого надо зарегистрироваться в @TrugRuBot!')
                                try:
                                ...
                                for ids in games[id]['bots']:
                                    try:
                                        if games[id]['bots'][ids]['identeficator'] == None:
                                            users.update_one({'id': games[id]['bots'][ids]['id']},
                                                             {'$inc': {'bot.exp': 2}})
                                            users.update_one({'id': games[id]['bots'][ids]['id']},
                                                             {'$inc': {'cookie': 2}})
                                    except:
                                ...
                                if games[id]['mode'] != 'dungeon':
                                    bot.send_message(id,
                                                     '🏆' + g + name + ' победил' + a + '! Но в режиме апокалипсиса призы не выдаются, играйте ради веселья! :)')
                                    if games[id]['mode'] == 'meteors':
                                        for ids in games[id]['bots']:
                                            if games[id]['bots'][ids]['identeficator'] == None:
                                                try:
                                                    users.update_one({'id': games[id]['bots'][ids]['id']},
                                                                     {'$inc': {'bot.meteorraingames': 1}})
                                                    users.update_one({'id': games[id]['bots'][ids]['id']}, {'$inc': {
                                                        'bot.takenmeteordmg': games[id]['bots'][ids][
                                                            'takenmeteordmg']}})
                                                    users.update_one({'id': games[id]['bots'][ids]['id']}, {'$inc': {
                                                        'bot.takenmeteors': games[id]['bots'][ids]['takenmeteors']}})
                                                except:
                                                    pass
                                else:
                                    dung = 1
                            ...
                            if games[id]['mode'] != 'dungeon':
                                bot.send_message(id,
                                                 '🏆' + name + ' победил! Но награду за победу можно получить только в официальном чате - @cookiewarsru!')
                            else:
                                dung = 1
                        if dung == 1:
                            z = 1
...
def dmgs(id):
    ...
    if games[id]['mode'] == 'farm':
        ...
        dead = []
        for ids in games[id]['bots']:
            if games[id]['bots'][ids]['die'] == 0 and games[id]['bots'][ids]['id'] != 'lava' and games[id]['bots'][ids][
                'id'] != 'sniper':
    ...
    for ids in games[id]['turrets']:
        ...
        for idss in games[id]['bots']:
            if games[id]['bots'][idss]['die'] != 1 and games[id]['bots'][idss]['hp'] > 0 and games[id]['bots'][idss][
                'id'] != ids and games[id]['bots'][idss]['zombie'] <= 0:
        ...
        if len(a) > 0:
            ...
            if yes == 1:
                ...
                if random.randint(1, 100) <= 40 * (1 + games[id]['bots'][ids]['chance']):
                    games[id]['res'] += '🔺Турель бойца ' + games[id]['bots'][ids]['name'] + ' стреляет в ' + trgt[
    ...
    for ids in games[id]['bots']:
        if 'firemage' in games[id]['bots'][ids]['skills']:
            if random.randint(1, 100) <= 18 + (18 * games[id]['bots'][ids]['chance']) and games[id]['bots'][ids][
                'die'] != 1:
                games[id]['bots'][ids]['firearmor'] = 1
                games[id]['res'] += '🔥Повелитель огня ' + games[id]['bots'][ids][
                    'name'] + ' использует огненный щит!\n'
    ...
    for ids in games[id]['bots']:
        ...
        if 'magictitan' in games[id]['bots'][ids]['skills'] and random.randint(1, 100) <= 50 + (
            ...
            if games[id]['bots'][ids]['magicshield'] > 0:
                ...
                if a > 0:
                    text += '🔵Магический титан ' + games[id]['bots'][ids]['name'] + ' блокирует ' + str(
                        a) + ' урона!\n'
                if games[id]['bots'][ids]['magicshield'] <= 0:
    ...
    for ids in games[id]['bots']:
        if games[id]['bots'][ids]['currentarmor'] > 0 and games[id]['bots'][ids]['takendmg'] > 0:
            text += '🔰Броня ' + games[id]['bots'][ids]['name'] + ' снимает ' + str(
                games[id]['bots'][ids]['currentarmor']) + ' урона!\n'
            games[id]['bots'][ids]['takendmg'] -= games[id]['bots'][ids]['currentarmor']

    for ids in games[id]['bots']:
        if 'suit' in games[id]['bots'][ids]['skills'] and random.randint(1, 100) <= 25 * (
                1 + games[id]['bots'][ids]['chance']) and games[id]['bots'][ids]['takendmg'] > 0 and \
                games[id]['bots'][ids]['target'] != None:
            games[id]['bots'][ids]['target']['takendmg'] += games[id]['bots'][ids]['takendmg']
            text += '📡' + games[id]['bots'][ids]['name'] + ' направляет полученный урон в свою цель! Нанесено ' + str(
                games[id]['bots'][ids]['takendmg']) + ' урона.\n'
    ...
    for mob in games[id]['bots']:
        ...
        if games[id]['bots'][mob]['blood'] != 0:
            ...
            if games[id]['bots'][mob]['blood'] == 0 and games[id]['bots'][mob]['die'] != 1 and games[id]['bots'][mob][
                'zombie'] <= 0:
            ...
            if games[id]['bots'][mob]['takendmg'] > 0:
                ...
                if games[id]['bots'][mob]['zombie'] == 0:
                    if games[id]['bots'][mob]['die'] != 1:
                        if 'oracle' not in games[id]['bots'][mob]['skin']:
                            ...
                            if games[id]['bots'][mob]['oracle'] >= 1 and games[id]['bots'][mob]['hp'] - a <= 0 and (
                                    xx <= 30 or games[id]['bots'][mob]['id'] == 'dungeon'):
                ...
                if games[id]['bots'][mob]['hp'] < 100:
                    ...
                    if cmob['id'] == 581167827:
                        ...
                        em_hp = '♥'
                    text += games[id]['bots'][mob]['name'] + ' Теряет ' + str(a) + ' хп. У него осталось ' + em_hp * \
                            games[id]['bots'][mob]['hp'] + str(games[id]['bots'][mob]['hp']) + 'хп!\n'
                    for idss in games[id]['bots']:
                        ...
                        if games[id]['bots'][idss]['target'] == games[id]['bots'][mob] and 'necromant' in \
                                games[id]['bots'][idss]['skills'] and random.randint(1, 100) <= 60 + (
                                60 * games[id]['bots'][idss]['chance']):
                            games[id]['bots'][idss]['summonmonster'][1] += a
                            text += '🖤Некромант ' + games[id]['bots'][idss]['name'] + ' прибавляет ' + str(
                                a) + ' хп к своему монстру!\n'
                        if unit['target'] == games[id]['bots'][mob] and 'werewolf' in unit['mutations'] and games[id][
                            'xod'] % 2 == 0 and random.randint(1, 100) <= 30:
                            text += unit['name'] + ' кусает цель и восстанавливает ' + em_hp + 'хп!\n'
                            unit['hp'] += 1
                else:
                    text += games[id]['bots'][mob]['name'] + ' Теряет ' + str(a) + ' хп. У него осталось ' + str(
                        games[id]['bots'][mob]['hp']) + 'хп!\n'
                    ...
                    if 'bloodmage' not in games[id]['bots'][mob]['skills']:
                        ...
                        if games[id]['bots'][mob]['name'] == 'Редкий слизнюк':
                            ...
                            for prize in games[id]['bots']:
                                if games[id]['bots'][prize]['target'] == games[id]['bots'][mob] and games[id]['bots'][
                                    prize] not in prizez:
                            ...
                            if len(prizez) > 0:
                                for pp in prizez:
                                    ...
                                    if 'sliznuk' not in pp['bought'] and random.randint(1, 100) <= 25:
                                        ...
                                        bot.send_message(pp['id'],
                                                         'Поздравляем, ваш боец поймал редкого слизнюка! Награда: 500❇/⚛, и уникальное оружие! Доступно оно будет в следующих обновлениях.')
                                    bot.send_message(pp['id'],
                                                     'Поздравляем, ваш боец поймал редкого слизнюка! Награда: 500❇/⚛.')
                                    text += pp['name'] + '\n'
                        if 'necromant' in games[id]['bots'][mob]['skills']:
                            monsters.append(games[id]['bots'][mob]['id'])
                        if games[id]['bots'][mob]['name'] != 'Редкий слизнюк':
                        ...
                        if randd <= 60 * (60 * games[id]['bots'][mob]['chance']):
                            a = []
                            for ids in games[id]['bots']:
                                if games[id]['bots'][ids]['die'] != 1 and games[id]['bots'][ids]['hp'] > 0 and \
                            ...
                            if len(a) > 0:
                                ...
                                if x2 != None:
                                    if x2['hp'] <= 0 or x1['hp'] <= 0:
                                        text += '🔥Маг крови ' + games[id]['bots'][mob][
                                            'name'] + ' перед смертью высасывает по жизни у ' + x1['name'] + ' и ' + x2[
                                                    'name'] + ', и воскресает с 2❤️!\n'
                                        ...
                                        text += '😵Маг крови ' + games[id]['bots'][mob][
                                            'name'] + ' перед смертью высасывает по жизни у ' + x1['name'] + ' и ' + x2[
                                                    'name'] + ', но никого не убивает, и погибает окончательно.\n'
                                        games[id]['bots'][mob]['dieturn'] = games[id]['xod']
                                        if games[id]['bots'][mob]['id'] == 'dungeon':
                                            if random.randint(1, 100) <= 2:
                                                tr = random.choice(games[id]['bots'][mob]['drops'])
                                    ...
                                    if x1['hp'] <= 0:
                                        text += '🔥Маг крови ' + games[id]['bots'][mob][
                                            'name'] + ' перед смертью высасывает жизнь у ' + x1[
                                                    'name'] + ', и воскресает с 2❤️!\n'
                                        games[id]['bots'][mob]['hp'] = 2
                                        text += '👹' + x1['name'] + ' теперь зомби!\n'
                                        x1['zombie'] = 1
                                        ...
                                        text += '😵Маг крови ' + games[id]['bots'][mob][
                                            'name'] + ' перед смертью высасывает жизнь у ' + x1[
                                                    'name'] + ', но не убивает цель, и погибает окончательно.\n'
                                        games[id]['bots'][mob]['dieturn'] = games[id]['xod']
                                        if games[id]['bots'][mob]['id'] == 'dungeon':
                                            if random.randint(1, 100) <= 2:
                                                tr = random.choice(games[id]['bots'][mob]['drops'])
    ...
    for mob in games[id]['bots']:
        if 'paukovod' in games[id]['bots'][mob]['skills'] and games[id]['bots'][mob]['die'] != 1 and \
...
def assasin(id, me, target):
    games[id]['res'] += '⭕Ассасин ' + me['name'] + ' достаёт револьвер и добивает ' + target[
        'name'] + ' точным выстрелом в голову!\n'
    target['hp'] -= 1
    ...
    if bot1['weapon'] == 'rock':
        ...
        if target['hp'] == 1 and 'cazn' in bot1['skills'] and target['zombie'] <= 0:
            ...
            if x * debuff / bonus <= chance or bot1['hit'] == 1:
                ...
                if bot1['zombie'] > 0:
                    damage += 3
                games[id]['res'] += '☄️' + bot1['name'] + ' Кидает камень в ' + target['name'] + '! Нанесено ' + str(
                    damage) + ' Урона.\n'
        ...
        if target['hp'] == 1 and 'cazn' in bot1['skills'] and target['zombie'] <= 0:
            ...
            if x * debuff / bonus <= chance or bot1['hit'] == 1:
                ...
                if bot1['zombie'] > 0:
                    damage += 3
                games[id]['res'] += '🔫' + bot1['name'] + ' Стреляет в ' + target['name'] + '! Нанесено ' + str(
                    damage) + ' Урона.\n'
                target['takendmg'] += damage
                target['takendmg'] += bot1['dopdmg']
        ...
        if target['hp'] == 1 and 'cazn' in bot1['skills'] and target['zombie'] <= 0:
            ...
            if x * debuff / bonus <= chance or bot1['hit'] == 1:
                ...
                if bot1['zombie'] > 0:
                    damage += 3
                games[id]['res'] += '🤜' + bot1['name'] + ' Бьет ' + target['name'] + '! Нанесено ' + str(
                    damage) + ' Урона.\n'
                target['takendmg'] += damage
                target['takendmg'] += bot1['dopdmg']
        ...
        if target['hp'] == 1 and 'cazn' in bot1['skills'] and target['zombie'] <= 0:
            ...
            if x * debuff / bonus <= chance or bot1['hit'] == 1:
                ...
                if bot1['zombie'] > 0:
                    damage += 3
                games[id]['res'] += '⚙️' + bot1['name'] + ' Стреляет в ' + target[
                    'name'] + ' из Пилострела! Нанесено ' + str(damage) + ' Урона.\n'
                target['takendmg'] += damage
                target['takendmg'] += bot1['dopdmg']
                bot1['energy'] -= 2
        ...
        if target['hp'] == 1 and 'cazn' in bot1['skills'] and target['zombie'] <= 0:
            ...
            if x * debuff / bonus <= chance or bot1['hit'] == 1:
                ...
                if bot1['zombie'] > 0:
                    damage += 3
                if target['reload'] != 1:
                    games[id]['res'] += '🗡' + bot1['name'] + ' Бъет ' + target['name'] + ' Кинжалом! Нанесено ' + str(
                        damage) + ' Урона.\n'
                    target['takendmg'] += damage
                    target['takendmg'] += bot1['dopdmg']
                    bot1['energy'] -= 2
                    ...
                    if a <= 100:
                        ...
                        if 'berserk' in bot1['skills'] and bot1['hp'] <= 2:
                            damage += 2
                        games[id]['res'] += '⚡️' + bot1['name'] + ' Наносит критический удар по ' + target[
                            'name'] + '! Нанесено ' + str(damage) + ' Урона.\n'
                        bot1['energy'] -= 5
                        target['takendmg'] += damage
                    else:
                        games[id]['res'] += '🗡' + bot1['name'] + ' Бъет ' + target[
                            'name'] + ' Кинжалом! Нанесено ' + str(damage) + ' Урона.\n'
                        target['takendmg'] += damage
                        bot1['energy'] -= 2
            else:
                games[id]['res'] += '💨' + bot1['name'] + ' Промахнулся по ' + target['name'] + '!\n'
        ...
        if target['hp'] == 1 and 'cazn' in bot1['skills'] and target['zombie'] <= 0:
            ...
            if bot1['bowcharge'] == 1:
                ...
                if x * debuff / bonus <= chance or bot1['hit'] == 1:
                    ...
                    if bot1['zombie'] > 0:
                        damage += 3
                    games[id]['res'] += '🏹' + bot1['name'] + ' Стреляет в ' + target[
                        'name'] + ' из лука! Нанесено ' + str(damage) + ' Урона.\n'
                    target['takendmg'] += damage
                    target['takendmg'] += bot1['dopdmg']
                    bot1['energy'] -= 5
        ...
        if target['hp'] == 1 and 'cazn' in bot1['skills'] and target['zombie'] <= 0:
            ...
            if x * debuff / bonus <= chance or bot1['hit'] == 1:
                ...
                if x <= 50:
                    stun = 1
                games[id]['res'] += '🕷' + bot1['name'] + ' кусает ' + target['name'] + '! Нанесено ' + str(
                    damage) + ' Урона.\n'
                if stun == 1:
        ...
        if target['hp'] == 1 and 'cazn' in bot1['skills'] and target['zombie'] <= 0:
            ...
            if x * debuff / bonus <= chance or bot1['hit'] == 1:
                ...
                if x <= 10:
                    eat = 1
                games[id]['res'] += '🦏' + bot1['name'] + ' бъёт ' + target['name'] + ' рогом! Нанесено ' + str(
                    damage) + ' Урона.\n'
                if eat == 1:
                    loss = 0
                    ...
                    games[id]['res'] += '👿' + bot1[
                        'name'] + ' в бешенстве! Он наносит критический удар по цели. Нанесено ' + \
                                        str(critdmg) + ' урона!\n' + '🌀' + bot1[
                                            'name'] + ' получает оглушение на ' + str(stunn - 1) + ' ход!\n'
                    bot1['stun'] = stunn
        ...
        if target['hp'] == 1 and 'cazn' in bot1['skills'] and target['zombie'] <= 0:
            ...
            if x * debuff / bonus <= chance or bot1['hit'] == 1:
                ...
                if x <= 18:
                    eat = 1
                games[id]['res'] += '💮' + bot1['name'] + ' накладывает порчу на ' + target[
                    'name'] + '! Нанесено ' + str(damage) + ' Урона.\n'
                if eat == 1:
                    enemys = []
                    for ids in games[id]['bots']:
                    ...
                    if target1 != target2:
                        games[id]['res'] += '☯' + bot1['name'] + ' связывает души ' + target1['name'] + \
                                            ' и ' + target2[
                                                'name'] + '! Каждый из них будет дополнительно получать урон другого ' + str(
                            boundtime - 1) + \
                                            ' следующих хода, включая этот!\n'
                    else:
                        games[id]['res'] += '☯' + bot1['name'] + ' проклинает душу ' + target1['name'] + '! ' + str(
                            boundtime - 1) + \
        ...
        if target['hp'] == 1 and 'cazn' in bot1['skills'] and target['zombie'] <= 0:
            ...
            if eat <= 5:
                ...
                eat = 0 \
                        if eat == 1:
                    games[id]['res'] += '🍗' + bot1[
                        'name'] + ' проголодался и решил закусить своей свинкой! Та теряет 1 хп.\n'
                for ids in games[id]['bots']:
        ...
        if target['hp'] == 1 and 'cazn' in bot1['skills'] and target['zombie'] <= 0:
            ...
            if bot1['zombie'] > 0:
                damage += 3
            games[id]['res'] += '🔯' + bot1['name'] + ' стреляет в ' + target[
                'name'] + ' из флюгегенхаймена! Нанесено ' + str(damage) + ' Урона.\n'
            target['takendmg'] += damage
            target['takendmg'] += bot1['dopdmg']
            bot1['energy'] -= 2
        ...
        if gun == 1:
            ...
            for ids in games[id]['bots']: \
                    if
            games[id]['bots'][ids]['id'] == bot1['id']:
            games[id]['bots'][ids]['deffromgun'] = 1
        games[id]['res'] += '☢' + bot1[
            'name'] + ' открыл слишком много порталов! Весь нанесённый в раунде урон будет перенаправлен в его случайного ' + \
                            'соперника!\n'
...
if target['hp'] == 1 and 'cazn' in bot1['skills'] and target['zombie'] <= 0:
    ...
    if flame <= 35:
        ...
        flame = 0
    games[id]['res'] += '💥' + bot1['name'] + ' поджигает ' + target['name'] + '! Нанесено ' + str(
        damage) + ' урона.\n'
    target['takendmg'] += damage
    target['fire'] += 2
    target['takendmg'] += bot1['dopdmg']
    ...
    if flame == 1:
        enm = []
        for ids in games[id]['bots']:
            if games[id]['bots'][ids]['id'] != bot1['id'] and games[id]['bots'][ids]['die'] != 1 and \
                    games[id]['bots'][ids] != target:
...
if target['hp'] == 1 and 'cazn' in bot1['skills'] and target['zombie'] <= 0:
    ...
    games[id]['res'] += '⚔' + bot1['name'] + ' рубит ' + target['name'] + '! Нанесено ' + str(
        damage) + ' урона.\n'
    target['takendmg'] += damage
    target['takendmg'] += bot1['dopdmg']
    bot1['energy'] -= 2
...
if target['hp'] == 1 and 'cazn' in bot1['skills'] and target['zombie'] <= 0: \
    ...
    games[id]['res'] += '💣' + bot1['name'] + ' стреляет в ' + target['name'] + ' из базуки! Нанесено ' + str(
        damage) + ' урона.\n'
    target['takendmg'] += damage
    target['takendmg'] += bot1['dopdmg']
    bot1['energy'] -= 7
...
if target['hp'] == 1 and 'cazn' in bot1['skills'] and target['zombie'] <= 0:
 ...
 \
    ...
    games[id]['res'] += '🦠' + bot1['name'] + ' стреляет в ' + target[
        'name'] + ' из слиземёта! Нанесено ' + str(damage) + ' урона по ' + str(i) + ' цели(ям)!\n'
    # target['takendmg']+=damage
    # target['takendmg']+=bot1['dopdmg']
    bot1['energy'] -= 2
...
if random.randint(1, 100) <= 15 and target['weapon'] != 'hand':
    games[id]['res'] += '♻' + bot1['name'] + ' поглощает оружие ' + target[
        'name'] + ', восстанавливая 2❤ хп! Теперь он будет сражаться кулаками!\n'
    target['weapon'] = 'hand'
    bot1['hp'] += 2
else: \
...
if target['hp'] == 1 and 'cazn' in bot1['skills'] and target['zombie'] <= 0:
    ...
    games[id]['res'] += '🥬' + bot1['name'] + ' кидает капусту в ' + target['name'] + '! Нанесено ' + str(
        damage) + ' урона.\n'
    target['takendmg'] += damage
    target['takendmg'] += bot1['dopdmg']
    bot1['energy'] -= 2
...
if target['hp'] == 1 and 'cazn' in bot1['skills'] and target['zombie'] <= 0:
    ...
    x = random.randint(1, 100) \
 \
            games[id]['res'] += '🉐' + bot1['name'] + ' бьёт ' + target['name'] + ' катаной! Нанесено ' + str(
                damage) + ' урона.\n'
    target['takendmg'] += damage
    target['takendmg'] += bot1['dopdmg']
    bot1['energy'] -= 2
...
if target['hp'] == 1 and 'cazn' in bot1['skills'] and target['zombie'] <= 0: \
    ...
    games[id]['res'] += '🦊Лиса бойца ' + bot1['name'] + ' кусает ' + target['name'] + '! Нанесено ' + str(
        damage) + ' урона.\n'
    target['takendmg'] += damage
    target['takendmg'] += bot1['dopdmg']
    bot1['energy'] -= 2
...
if target['hp'] == 1 and 'cazn' in bot1['skills'] and target['zombie'] <= 0:
    ...
    x = random.randint(1, 100)
    games[id]['res'] += '🌝' + bot1['name'] + ' стреляет в ' + target[
        'name'] + ' из емоджимёта! В соперника прилетает "' + em + '"! Нанесено ' + str(damage) + ' урона.\n'
    target['takendmg'] += damage
    target['takendmg'] += bot1['dopdmg']
    bot1['energy'] -= random.randint(1, 3)

else:
    games[id]['res'] += '💨' + bot1['name'] + ' промахнулся по ' + target[
        'name'] + '! Мимо пролетает "' + em + '"!\n'
    bot1['target'] = None
    bot1['energy'] -= random.randint(1, 3)
...
def secondsliz(target, id, bot1):
    lst = []
    for ids in games[id]['bots']:
        if games[id]['bots'][ids]['target'] == target and games[id]['bots'][ids] not in lst and games[id]['bots'][ids][
            'id'] != bot1:
            lst.append(games[id]['bots'][ids])
    return lst
...
def naeb(bot, target, id):
    ...
    games[id]['res'] += '😯' + bot['name'] + ' атакует наебателя ' + target[
        'name'] + ', но тот наёбывает его! Вся энергия атаковавшего (' + str(bot['energy']) + ') оказывается у ' + \
                        enemy['name'] + '!\n'
...
def reload(bot2, id):
    ...
    if bot2['weapon'] == 'rock' or bot2['weapon'] == 'hand' or bot2['weapon'] == 'magic' or bot2[
        'weapon'] == 'kinzhal' or \
            bot2['weapon'] == 'sliznuk' or bot2['weapon'] == 'sword':
        games[id]['res'] += '😴' + bot2['name'] + ' отдыхает. Энергия восстановлена до ' + str(
            bot2['maxenergy']) + '!\n'
    elif bot2['weapon'] == 'bite':
        games[id]['res'] += '😴' + bot2['name'] + ' восполняет запасы яда. Энергия восстановлена до ' + str(
            bot2['maxenergy']) + '!\n'
    else:
        games[id]['res'] += '🕓' + bot2['name'] + ' перезаряжается. Энергия восстановлена до ' + str(
            bot2['maxenergy']) + '!\n'
    ...
    if len(a) > 0:
        ...
        if 'gipnoz' in bot['mainskill']:
            ...
            for ii in games[id]['bots']:
                if games[id]['bots'][ii]['energy'] >= 3 and games[id]['bots'][ii]['magicshieldkd'] <= 0 and \
                        games[id]['bots'][ii]['die'] == 0 and games[id]['bots'][ii]['id'] != bot['id'] and (
                        (games[id]['bots'][ii]['weapon'] == 'bow' and games[id]['bots'][ii]['bowcharge'] == 1) or
                        games[id]['bots'][ii]['weapon'] != 'bow'):
    ...
    if choice == 'medic':
        if bot['heal'] <= 0:
            a = random.randint(1, 100) \
                    if a < 75 and random.randint(1, 100) > 25:
                        ...
                        games[id]['res'] += '💔Медик ' + bot[
                            'name'] + ' неправильно сделал себе укол! Он теряет 1 хп.\n'
                        bot['heal'] = 10
                        bot['hp'] -= 1
        ...
        if target != None:
            if len(target['skills']) > 0:
                ...
                bot['shockcd'] = 8
                games[id]['res'] += '✴️' + bot['name'] + ' выпускает мощный поток энергии в ' + target[
                    'name'] + '! Тот теряет скилл "' + skilltoname(skill) + '"!\n'
                if skill == 'liveful':
                    target['hp'] -= 2
                    target['accuracy'] += 20
                if skill == 'dvuzhil':
                ...
                games[id]['res'] += '✴️' + bot['name'] + ' выпускает мощный поток энергии в ' + target[
                    'name'] + '! У него не было скиллов, поэтому он теряет 💔 хп!\n'
                target['hp'] -= 1
...
def actnumber(bot, id):
    ...
    for mob in enemy1:
        if mob['id'] != npc['id']:
            enemy.append(mob)
    for mob in enemy:
        if mob['energy'] <= 2 or mob['stun'] > 0 or (mob['weapon'] == 'magic' and mob['animal'] == 'pig') or mob[
            'die'] == 1 or (mob['weapon'] == 'bow' and mob['bowcharge'] == 0) or mob['magicshieldkd'] > 0:
            low += 1
    if low >= len(enemy):
        yvorot = 0 \
                else:
    ...
    if attack == 0 and yvorot == 0 and item == 0 and skill == 0:
        ...
        reload = 0
    return {'attack': {'name': 'attack', 'x': attack}, 'yvorot': {'name': 'yvorot', 'x': yvorot},
            'item': {'name': 'item', 'x': item}, 'reload': {'name': 'reload', 'x': reload},
            'skill': {'name': 'skill', 'x': skill}}
                        ...
                        if y['bot']['id'] not in games[int(x[0])]['ids']:
                            if y['bot']['name'] != None:
                                ...
                                if join == 1:
                                    ...
                                    bot.send_message(int(x[0]), m.from_user.first_name + ' (боец ' + thisbot[
                                        'name'] + ') присоединился!')
                                    games[int(x[0])]['ids'].append(m.from_user.id)
                                else: \
                                        bot.send_message(m.chat.id,
                                                         'Нельзя находиться в другом данже, если вы собираетесь идти в новый!')
                            else:
    ...
    if users.find_one({'id': m.from_user.id}) == None:
        try:
            bot.send_message(m.from_user.id,
                             'Здраствуйте, вы попали в игру "CookieWars"! Вам был выдан начальный боец. В будущем вы сможете улучшить его за поинты! Подробнее об игре можно узнать с помощью команды /help.')
        ...
        if i == 1:
            ...
            users.update_one({'id': m.from_user.id}, {'$set': {'inviter': int(z[1])}}) \
                    try:
                        bot.send_message(int(z[1]),
                                         'По вашей ссылке зашёл пользователь ' + m.from_user.first_name + '! По мере достижения им званий вы будете получать за него бонус - половину от его награды за звание.')
                    except:
                        pass
    ...
    try:
        if m.chat.id in games:
            if games[m.chat.id]['enablestart'] == 1 or m.from_user.id == 441399484:
                if len(games[m.chat.id]['bots']) >= 2 or (
                        games[m.chat.id]['mode'] == 'dungeon' and len(games[m.chat.id]['bots']) >= 1):
                    if games[m.chat.id]['started'] == 0:
                        if (
                                m.chat.id == -1001208357368 or m.chat.id == -1001172494515 or m.chat.id == -1001488903839) and m.from_user.id != 441399484:
...
def withoutauto(m):
    ...
    if m.chat.id not in games:  # and m.from_user.id==441399484:
        ...
        t.start()
        kb = types.InlineKeyboardMarkup()
        kb.add(types.InlineKeyboardButton(text='Присоединиться',
                                          url='telegram.me/cookiewarsbot?start=' + str(m.chat.id) + '_' + str(code)))
...
def withoutauto(m):
    ...
    if m.chat.id not in games:  # and m.from_user.id==441399484:
        ...
        kb = types.InlineKeyboardMarkup()
        kb.add(
            types.InlineKeyboardButton(text='Присоединиться', url='telegram.me/cookiewarsbot?start=' + str(m.chat.id)))
...
def apocalypse(m):
    ...
    if m.chat.id not in games:  # and m.from_user.id==441399484:
        ...
        t.start()
        kb = types.InlineKeyboardMarkup()
        kb.add(types.InlineKeyboardButton(text='Умереть',
                                          url='telegram.me/cookiewarsbot?start=' + str(m.chat.id) + '_' + str(code)))
...
def begin(m):
    ...
    if y['enablegames'] == 1:
        if m.chat.id not in games:
            ...
            kb = types.InlineKeyboardMarkup()
            kb.add(types.InlineKeyboardButton(text='Присоединиться',
                                              url='telegram.me/cookiewarsbot?start=' + str(m.chat.id) + '_' + str(
                                                  code)))
            ...
            if m.chat.id == -1001208357368:
                ...
                for ids in x:
                    if ids['id'] != 0:
                        if ids['enablejoin'] == 1 and ids['joinbots'] > 0 and ids['bot']['name'] != None:
                            ...
                            try:
                                text += ids['name'] + ' (боец ' + ids['bot'][
                                    'name'] + ') присоединился! (🤖Автоджоин)\n'
                            except:
                                pass
                try:
                    bot.send_message(m.chat.id, text)
                ...
                for ids in x:
                    if ids['id'] != 0:
                        if ids['nomutantjoin'] == 1 and ids['joinbots'] > 0 and ids['bot']['name'] != None and \
                                ids['bot']['mutations'] == []:
                            games[m.chat.id]['bots'].update(createbott(ids['id'], ids['bot']))
                            games[m.chat.id]['ids'].append(ids['id'])
                            users.update_one({'id': ids['id']}, {'$inc': {'joinbots': -1}})
...
def createpyro(chatid, id='dungeon'):
    ...
    strenght = 1.3
    hp = 4
    return createunit(id=id, drops=['ring_of_fire', 'magmaball'], weapon='flame', name=text, hp=hp, maxhp=hp,
                      identeficator=x, damagelimit=6, strenght=strenght, skills=['pricel', 'berserk', 'bloodmage'])
...
def createhypnotist(chatid, id='dungeon'):
    ...
    strenght = 1.5
    hp = 6
    return createunit(id=id, drops=['eye_of_seeing', 'hypnogun'], weapon='saw', name=text, hp=hp, maxhp=hp,
                      identeficator=x, damagelimit=8, strenght=strenght, skills=['gipnoz', 'liveful', 'metalarmor'])
...
def createseer(chatid, id='dungeon'):
    ...
    strenght = 2
    hp = 1
    return createunit(id=id, drops=['stone_of_life', 'magic_essense'], weapon='kinzhal', name=text, hp=hp, maxhp=hp,
                      identeficator=x, damagelimit=6, strenght=strenght, skills=['zeus', 'cazn', 'zombie'], oracle=5,
                      skin=['oracle'])
...
def createwarrior(chatid, id='dungeon'):
    ...
    strenght = 1
    hp = 8
    return createunit(id=id, drops=['helmet_of_the_strenght', 'magic_sword'], weapon='hand', name=text, hp=hp, maxhp=hp,
                      identeficator=x, damagelimit=10, strenght=strenght)
...
def createskeleton(chatid, id='dungeon'):
    ...
    strenght = 2.3
    hp = 6
    return createunit(id=id, drops=['magic_bone_wand', 'bonegun'], weapon='sword', name=text, hp=hp, maxhp=hp,
                      identeficator=x, damagelimit=999, strenght=strenght,
                      skills=['shieldgen', 'nindza', 'double', 'firemage', 'berserk'])
...
def begingame(id):
    try:
        ...
        if id == -1001208357368 and random.randint(1, 100) == 1:
            ...
            bot.send_message(id, 'На поле боя был замечен **редкий слизнюк**! Кто поймает его, тот получит 500❇/⚛!',
                             parse_mode='markdown')
            for ids in games[id]['bots']:
                try:
                    bot.send_message(games[id]['bots'][ids]['id'],
                                     'Редкий слизнюк был замечен на поле битвы! Заходите в чат @cookiewarsru, чтобы посмотреть, кто его поймает!')
                except:
                    pass
        ...
        for ids in createlist:
            ...
            text2 += '🎭' + ids['name'] + ' призывает своего двойника! У каждого из них по ' + str(
                ids['hp']) + ' хп!\n' \
        ...
        for ids in games[id]['bots']:
            if games[id]['bots'][ids]['weapon'] == 'magic':
                ...
                tt2 += 'Волшебная палочка бойца ' + games[id]['bots'][ids][
                    'name'] + ' превращает его в случайное существо: ' + animalname + '!\n\n'
        try:
            if tt2 != '':
    ...
    if m.from_user.id == 441399484:
        ...
        try:
            ...
            for ids in games[-1001208357368]['bots']:
                if games[-1001208357368]['bots'][ids]['id'] == id and games[-1001208357368]['bots'][ids][
                    'identeficator'] == None:
            ...
            if len(word) == 2:
                try:
                    ...
                    if price >= 20:
                        ...
                        bot.send_message(m.chat.id, 'Для совершения покупки поинтов, отправьте ' + str(
                            word[1]) + ' рубль(ей) на киви-кошелёк по логину:\n' +
                                         '`egor5q`\nС комментарием:\n`' + str(
